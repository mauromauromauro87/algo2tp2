...
\algoritmo{iNuevosVecinos}{in r:redstr, in camino:$Lista(compu)$, in visitados:$Lista(compu)$}{res : $Lista(compu)$}{
	...
	\State \Var{vecinos:$Lista$} $\larr$ $significados(significado(r.conexiones,camino[long-1]))$ \complejidad{O(1)}
	...
	\While {$haySiguiente$} \complejidad{O($\#(r.compus)$)}
		\State \Var{vecino:$compu$} $\larr$ $actual(itVecinos)$ \complejidad{O(1)}
		\If {$\neg(Esta(visitados,vecino))$} \complejidad{O(1)}
