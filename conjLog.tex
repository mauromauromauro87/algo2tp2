...
\algoritmo{iBorrar}{in/out cl:conjLog($\alpha$), in e : $\alpha$}{}{
	...
	\State \If {$(\neg(cl.der==NULL) \land \neg(cl.izq==NULL))$} \complejidad{O(1)}
		...
		\EndIf
		\State $mm \larr {\Fun{iDameMayorMenores}(clactual)}$ \complejidad{O($log(size(cl))$)}
		\State \If {$mm.valor == e$} \complejidad{O(1)}
			...
			\EndIf
		\Else
			\State $mmValor:\alpha \larr mm.valor$ \complejidad{O(1)}
...
\algoritmo{iMenor}{in cl:conjLog($\alpha$)}{res:$\alpha$}{
	\State $clactual:conjLog(\alpha) \larr cl$ \complejidad{O(1)}
...
\algoritmo{iBuscar}{in cl:conjLog($\alpha$), e:$\alpha$}{res:$\alpha$}{
	\State $padre:conjLog(\alpha) \larr iEncontrarPadre(cl,e)$ \complejidad{O($log(size(cl))$)}
...
\algoritmo{iRebYRecalcFDB}{in/out cl:conjLog($\alpha$), in variandoHijoDerecho?:bool, in estoyBorrando?:bool}{}{
	\State $clactual = cl$ \complejidad{$O(1)$}
	\State \If {$estoyBorrando?$}
		\State \While {$clactual != NULL \land \neg(termino?)$} \complejidad {$O(1)$}
			...
			\Else
				\State \If{$clactual.fdb == -1$} \complejidad{$O(1)$}
					\State $fdbDer:nat$ \complejidad{O(1)}
					\State \If{$cl.der != NULL$} \complejidad{O(1)}
...
\algoritmo{iRotarRR}{in/out cl:conjLog($\alpha$)}{}{
	\State $nietoRR \larr cl.der.der$ \complejidad{O(1)}
	\State $hijoDer \larr cl.der$ \complejidad{O(1)}
	\State $hijoIzq \larr cl.izq$ \complejidad{O(1)}
	\State $cl.der \larr NULL$ \complejidad{O(1)} 
...
\algoritmo{iRotarRL}{in/out cl:conjLog($\alpha$)}{}{
	\State $nietoRR:conjLog(\alpha) \larr cl.der.der$ \complejidad{O(1)}
	\State $nietoRL:conjLog(\alpha) \larr cl.der.izq$ \complejidad{O(1)}
	\State $valorDer:\alpha \larr cl.der.valor$ \complejidad{O(1)}
...
\algoritmo{iRotarLL}{in/out cl:conjLog($\alpha$)}{}{
	\State $nietoLL \larr cl.izq.izq$ \complejidad{O(1)}
	\State $hijoIzq \larr cl.izq$ \complejidad{O(1)}
	\State $hijoDer \larr cl.der$ \complejidad{O(1)}
	\State $cl.izq \larr NULL$ \complejidad{O(1)} 
...
\algoritmo{iRotarLR}{in/out cl:conjLog($\alpha$)}{}{
	\State $nietoLL:conjLog(\alpha) \larr cl.izq.izq$ \complejidad{O(1)}
	\State $nietoLR:conjLog(\alpha) \larr cl.izq.der$ \complejidad{O(1)}
	\State $valorIzq:\alpha \larr cl.izq.valor$ \complejidad{O(1)}
...
\algoritmo{iEncontrarPadre}{in cl:conjLog($\alpha$), e:$\alpha$}{res:conjLog($\alpha$)}{
	\State $clactual:conjLog(\alpha)$ \complejidad{O(1)}
	\State $encontrado?:bool \larr (clactual.der != NULL \yluego clactual.der.valor == e) \lor (clactual.izq !=NULL \yluego clactual.izq.valor == e)$
	\State \While {$\neg encontrado?$} 
